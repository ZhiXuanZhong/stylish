...
function genListing(listingData) {
  ...
  data.forEach((item) => {
    const { main_image:mainImage, colors, title, price, id } = item
    // const {mainImage: mainImage} =  item

    let colorsHTML = ''
    colors.forEach((color) => (colorsHTML += `<span class="item__color" style="background-color:#${color.code};"></span>`))

    const html = `
    <div class="procuct__item">
      <a href="/product?id=${id}">
        <img class="item__image" src="${mainImage}"
            alt="${title}">
...
function renderData() {
  if (Object.hasOwn(viewState, 'q')) {
    ...
    dataFetcher(genEndpoint('q', viewState.q, lastFetch.next_paging)).then((newListing) => {
  ...
  if (Object.hasOwn(viewState, 'category')) {
    //Update data to reflex on UI style
    dataFetcher(genEndpoint('category', viewState.category, lastFetch.next_paging)).then((newListing) => {
      genListing(newListing)
    })
    updateCateStyle()
    delete viewState.q
